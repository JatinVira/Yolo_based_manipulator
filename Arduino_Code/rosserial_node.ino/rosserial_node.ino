...
2. When a message is received, check the color and move the servo
accordingly
3. There are a total of 6 Servo Motors, All of them move to move the arm
...
7. The Arm will have all the 6 joint angles fixed for each drop position
and pickup position
8. The Arm will also have additional default position that its in after
the pickup and drop is complete
9. The Arm will also have an intermediate position that it will move to
between the pickup and drop position
10. The program has to simply move the arm to the pickup position, then
to the intermediate position, then to the drop position and then back to
the default position
11. The program has to check the color of the box and move the arm
accordingly.
12. Also setup a publisher to publish the current status of the arm on
the topic "arm_status"
*/
